...
class CTkSettings(customtkinter.CTkToplevel):
    def __init__(self, parent, settings=None,  font=None):
        ...
        if settings is None:
            CTkMessagebox(title="Error", message="Program error, settingsDialog initialization error", icon="cancel", font=font)
            return
        if not isinstance(settings, dict):
            CTkMessagebox(title="Error", message='Program error, settings must be a dictionary', icon='cancel', font=font)
            return
        ...
        self.weight_switch = customtkinter.CTkSwitch(self.font_option_frame, text='bold', variable=self.weight_var, onvalue='bold', offvalue= 'normal', command=self.on_switch_weight, font=font)
        ...
        self.italic_switch = customtkinter.CTkSwitch(self.font_option_frame, text='Italic', variable=self.italic_var, onvalue='italic', offvalue= 'roman', command=self.on_switch_italic, font=font)
        ...
        self.underline_switch = customtkinter.CTkSwitch(self.font_option_frame, text='Underline', variable=self.underline_var, onvalue='yes', offvalue= 'no', command=self.on_switch_underline, font=font)
        ...
        self.overstrike_switch = customtkinter.CTkSwitch(self.font_option_frame, text='Overstrike', variable=self.overstrike_var, onvalue='yes', offvalue= 'no', command=self.on_switch_overstrike, font=font)
    ...
    def on_configure_font(self):
        ...
        fp = FontPicker(self, title='Configure Font', current_font=font)
    ...
    def update_theme(self):
        ...
        dialog = CustomAskYesNo(self, message="Do you want to select a new theme?", title="Confirmation", font=self.font)
        if dialog.result:
            theme_name=customtkinter.filedialog.askopenfilename()
            self.settings['theme'] = theme_name
            # self.save_settings()
            CustomDialog(self, 'Caveat', 'Theme will be used after restarting the app', font=self.font)
